	...
	def control_pid(self):
		...
		while not rospy.is_shutdown():
			...
			if self.fuzzyfy:
				...
				KPx = self.defuzzed.P_X
				#if KPx <=0: KPx=0
				#if KPx >=2.6: KPx=2.6
				global KIx
				#KIx += self.defuzzed.I_X/SOFTNESS
				KIx = KPx*KPx/(self.defuzzed.I_X*self.defuzzed.D_X)
				#if KIx <= 0: KIx = 0
				#if KIx >= 1.2: KIx = 1.2
				global KDx
				#KDx += self.defuzzed.D_X/SOFTNESS
				KDx = self.defuzzed.D_X
				#if KDx <= 0: KDx = 0
				#if KDx >= 1.4: KDx = 1.4
			...
			fuz_msg.Error = abs(self.error.x.act)
			fuz_msg.D_Error = self.error.x.Deri2 / _time_bet_run
			fuz_msg.P_val = KPx
			fuz_msg.I_val = KIx
			fuz_msg.D_val = KDx
			...
			ref_pose_msg.pose.position.y = 0
			ref_pose_msg.pose.position.z = setpoint_vel.z
			ref_pose_msg.pose.position.z = 0
