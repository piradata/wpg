...
#PID constantes
_KPo = KPx = KPy = KPz = 3.300
_KIo = KIx = KIy = KIz = 3.000
_KDo = KDx = KDy = KDz = 0.330
	...
	def control_pid(self):
		...
		while not rospy.is_shutdown():
			...
			if self.fuzzyfy:
				...
				global KPx
				KPx = _KPo + self.defuzzed.P_X/SOFTNESS
				if KPx <=0.6: KPx=0.6
				if KPx >=5.6: KPx=5.6
				global KIx
				KIx = _KIo + self.defuzzed.I_X/SOFTNESS
				if KIx <= 0.3: KIx = 0.3
				if KIx >= 7: KIx = 7
				global KDx
				KDx = _KDo + self.defuzzed.D_X/SOFTNESS
				if KDx <= 0.1: KDx = 0.1
				if KDx >= 0.7: KDx = 0.7
